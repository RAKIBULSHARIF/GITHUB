                     ...
                     int *distance )
{
    ...
    *distance = 0;
    ...
    /*find path from source to destination vertex*/
    sourceNode = source;
    do /*until destination vertex is not found*/
    {
        for ( i = 1; i < vertices + 1; i++ )
        {
            ...
            if (( adjacency[sourceNode][i] > 0 ) &&
                (pathInfo[i].mark == GRAY ))
            {
                if ((pathInfo[sourceNode].distance +
                     adjacency[sourceNode][i] ) < pathInfo[i].distance )
                {
                    pathInfo[i].predecessor = sourceNode;
                    pathInfo[i].distance = pathInfo[sourceNode].distance +
                    adjacency[sourceNode][i];
                }
            }
        }
        ...
        for ( i = 1; i < vertices + 1; i++ )
        {
            if (( pathInfo[i].mark == GRAY) &&
                ( pathInfo[i].distance < minDistance ))
            {
                minDistance = pathInfo[i].distance;
                sourceNode = i;
            }
        }
        /*check whether source or destination vertices
        are not isolated*/
        if ( sourceNode == 0 )
        {
            return 0;
...
int main()
{
    ...
    printf("    to: ");
    scanf("%d",&to);
    nodes = findShortestPath( adjacent,
                             n,
                             from,
                             to,
                             shortestPath,
